...
contract DogeSon is Context, IBEP20, Ownable {
    ...
    uint256 public _liquidityFee = 400; // 4%

    uint256 public _totalFeeForBNB = _buyBackFee + _marketingFee + _devFee + _liquidityFee; // Percentages for liquidity, buyback, marketing and dev (NOTE: liquidity is 4%, buyback is 2%, marketing is 4% and dev is 4%)
    ...
    function setAllFeePercents(uint256 buyBackFee, uint256 marketingFee, uint256 devFee, uint256 liquidityFee) public onlyOwner() {
        _buyBackFee = buyBackFee;
        _marketingFee = marketingFee;
        _devFee = devFee;
        _liquidityFee = liquidityFee;
        _totalFeeForBNB = _buyBackFee.add(_marketingFee).add(_devFee).add(_liquidityFee);
    }

    function setBuyBackFeePercent(uint256 value) external onlyOwner() {
        _totalFeeForBNB = _totalFeeForBNB.sub(_buyBackFee);
        _buyBackFee = value;
        _totalFeeForBNB = _totalFeeForBNB.add(_buyBackFee);
    }

    function setMarketingFeePercent(uint256 value) external onlyOwner() {
        _totalFeeForBNB = _totalFeeForBNB.sub(_marketingFee);
        _marketingFee = value;
        _totalFeeForBNB = _totalFeeForBNB.add(_marketingFee);
    }

    function setDevFeePercent(uint256 value) external onlyOwner() {
        _totalFeeForBNB = _totalFeeForBNB.sub(_devFee);
        _devFee = value;
        _totalFeeForBNB = _totalFeeForBNB.add(_devFee);
    }

    function setLiquidityFeePercent(uint256 value) external onlyOwner() {
        _totalFeeForBNB = _totalFeeForBNB.sub(_liquidityFee);
        _liquidityFee = value;
        _totalFeeForBNB = _totalFeeForBNB.add(_liquidityFee);
    }
    ...
    function endPreSaleDiscount() external onlyOwner {
        ...
        _taxFee = 100;
        setAllFeePercents(200, 400, 400, 400);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tTotalFeeForBNB) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tTotalFeeForBNB, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tTotalFeeForBNB);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 forLiquidity = contractTokenBalance.mul(_liquidityFee).div(_totalFeeForBNB);
        ...
        uint256 tokensToBeSwapped = contractTokenBalance.sub(half);

        // Take the first initial BNB balance
        uint256 initialBNBBalance = address(this).balance;

        // Swap tokens to BNB
        swapTokensForEth(tokensToBeSwapped);
        ...
        uint256 eachPart = swappedBNB.div(_totalFeeForBNB.sub(_liquidityFee.div(2)));
        uint256 swappedBNBForLiquidity = eachPart.mul(_liquidityFee.div(2));
